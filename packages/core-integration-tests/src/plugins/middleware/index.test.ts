...
describe(sourceMiddlewarePlugin, () => {
  ...
  it('is loaded automatically', async () => {
    // @ts-expect-error
    expect(await xt.load(CoreContext.system())).toBeTruthy()
  ...
  describe('Middleware', () => {
    ...
    it('cancels the event if `next` is not called', async () => {
      ...
      const toReturn = new CoreContext({ type: 'track' })
      const returnedCtx = await doesNotHangXT.track!(toReturn)

      expect(returnedCtx).toBe(toReturn)

      const toCancel = new CoreContext({ type: 'track' })
      await Promise.resolve(hangsXT.track!(toCancel)).catch((err) => {
  ...
  describe('Common use cases', () => {
    it('can be used to cancel an event altogether', async () => {
      ...
      // TODO: make browser use CoreContext
      // @ts-ignore
    ...
    it('can be used to re-route/cancel destinations', async () => {
      ...
      const gaDestination: CorePlugin = {
